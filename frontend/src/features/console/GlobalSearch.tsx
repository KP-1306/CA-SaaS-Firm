...
type SearchTarget = 'clients' | 'team' | 'work' | 'services';
...
export function GlobalSearch({
  ...
  const results = useMemo<SearchResult[]>(
    () => groups.flatMap((group) => group.rows.map((row) => ({
      ...
      id: String(row.id ?? `${group.key}-${resultTitle(group.key, row)}`),
      title: resultTitle(group.key, row),
      subtitle: resultSubtitle(group.key, row),
    }))),
    [groups],
  );
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      Promise.all([
        ...
        .then(([clients, employees, work, services]) => {
          if (cancelled) return;
    ...
    <div className="cx-global-search" ref={wrapperRef}>
      <div className="cx-global-search-box">
        ...
        <input
          ...
          type="search"
          value={query}
          placeholder="Search clients, employees, work or services"
